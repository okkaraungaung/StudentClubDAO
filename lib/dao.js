...
export const CONTRACT_ADDRESS = "0x35c17FB4273D17eE4a5efB6925a6B84411C27688";
...
const loadLogsInChunks = async (contract, filterFactory, provider, chunkSize = 4000) => {
...
export async function loadMemberDirectory(
  ...
  const [
    memberAddedResult,
    memberRemovedResult,
    overdueRemovedResult,
    membershipFeeResult,
    onChainMemberCountResult,
    periodsResult,
  ] = await Promise.allSettled([
    loadLogsInChunks(contract, () => contract.filters.MemberAdded(), provider),
    loadLogsInChunks(contract, () => contract.filters.MemberRemoved(), provider),
    loadLogsInChunks(
      contract,
      () => contract.filters.OverdueMemberRemoved(),
      provider,
    ),
    contract.membershipFee(),
    contract.memberCount(),
    cachedPeriods ? Promise.resolve(cachedPeriods) : loadPaymentPeriods(contract),
  ]);

  const memberAddedLogs =
    memberAddedResult.status === "fulfilled" ? memberAddedResult.value : [];
  const memberRemovedLogs =
    memberRemovedResult.status === "fulfilled" ? memberRemovedResult.value : [];
  const overdueRemovedLogs =
    overdueRemovedResult.status === "fulfilled"
      ? overdueRemovedResult.value
      : [];
  const membershipFee =
    membershipFeeResult.status === "fulfilled" ? membershipFeeResult.value : 0n;
  const onChainMemberCount =
    onChainMemberCountResult.status === "fulfilled"
      ? onChainMemberCountResult.value
      : 0n;
  ...
  for (const log of memberRemovedLogs) {
    ...
    removalLookup.set(address.toLowerCase(), {
      ...
      timestamp: await getBlockTimestamp(provider, log.blockNumber, blockCache),
    });
  }
  ...
  for (const [normalizedAddress, address] of addressLookup.entries()) {
    ...
    const addedInfo = joinedLookup.get(normalizedAddress) || null;
    const removalInfo = removalLookup.get(normalizedAddress) || null;
    const joinedAt = addedInfo
      ? formatDateLabel(addedInfo.timestamp)
      : joinedPeriodId === 0
        ? "Deployment"
        : joinedPeriod
    ...
    } else if (joinedPeriodId > 0 && effectiveSelectedPeriodId < joinedPeriodId) {
    ...
    memberEntries.push({
      ...
      removalAt: removalInfo?.timestamp ? formatDateLabel(removalInfo.timestamp) : "",
